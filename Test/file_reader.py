...
class FileReader(object):
    ...
    def skip_bytes(self, read_bytes):
        self._file_pointer.seek(read_bytes,1)
        print('Skipped %dGB'%(read_bytes//1024**3))
    ...
    def read_data_skip(self, read_bytes, isloop, dtype):
        if read_bytes > self._max_bytes:
            read_bytes=self._max_bytes
        if self._current_bytes + read_bytes <= self._max_bytes:
            self.skip_bytes(read_bytes)
            self._current_bytes += read_bytes
        else:
            if isloop==False:
                self._file_pointer=open(self._file_name, 'rb')
                self._file_pointer.read(read_bytes)
                self._current_bytes = read_bytes
            else:
                self._file_pointer=open(self._file_name, 'rb')
                self._file_pointer.read(read_bytes-(self._max_bytes-self._current_bytes))
                self._current_bytes = read_bytes-(self._max_bytes-self._current_bytes)
