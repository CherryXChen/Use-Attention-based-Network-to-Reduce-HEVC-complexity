...
DATA_SWITCH = input_data_LSTM.DATA_SWITCH

RUN_MODE = 1 # 0: train  1: evaluate
...
EVALUATE_QP_THR_LIST = input_data_LSTM.EVALUATE_QP_THR_LIST
MODEL_NAME = input_data_LSTM.MODEL_NAME
...
def get_class_matrices(y_truth, y_predict_64, y_predict_32, y_predict_16, thr_list):
    ...
    for i in range(num_samples):
        class_64_truth=is_sep_64(y_truth[i], input_data_LSTM.DEFAULT_THR_LIST[0])
        class_64_predict=is_sep_64(y_predict_64[i], thr_list[0])
        matrix_64[class_64_truth][class_64_predict] += 1
        if class_64_truth==1:
            for j in range(4):
                class_32_truth=is_sep_32(y_truth[i][index_32_list[j]], input_data_LSTM.DEFAULT_THR_LIST[1])
                class_32_predict=is_sep_32(y_predict_32[i][j], thr_list[1])
                matrix_32[class_32_truth][class_32_predict] += 1
                if class_32_truth==1:
                    for k in range(4):
                        class_16_truth=is_sep_16(y_truth[i][index_32_list[j][k]], input_data_LSTM.DEFAULT_THR_LIST[2])
...
def get_accuracy_on_large_data_interqp(f, data_set, qp_thr_list, thr_list, plot_tag_set=None):
    range_stat=input_data_LSTM.RangingStatistics(qp_thr_list, 'scalar')
    qps = data_set.efs[:,1]
...
def evaluate_loss_accuracy(step,learning_rate_value):
    ...
    train_y_truth_temp,train_y_predict_temp_64,train_y_predict_temp_32,train_y_predict_temp_16,train_accuracy_list_c, train_loss_list_c = sess.run([y_truth_valid,y_pred_flat_64,y_pred_flat_32,y_pred_flat_16, accuracy_list, loss_list],feed_dict={x:train_batch[0], y_:train_batch[1], ef:train_batch[2], isdrop:0, batch_size:NUM_TRAIN_PART})			
    valid_y_truth_temp,valid_y_predict_temp_64,valid_y_predict_temp_32,valid_y_predict_temp_16,valid_accuracy_list_c, valid_loss_list_c = sess.run([y_truth_valid,y_pred_flat_64,y_pred_flat_32,y_pred_flat_16, accuracy_list, loss_list],feed_dict={x:valid_batch[0], y_:valid_batch[1], ef:valid_batch[2], isdrop:0, batch_size:NUM_TRAIN_PART})
...
def evaluate(is_optimize=False):	
    ...
    test_end = time.time()
    print('test_begin is: ', time.asctime(time.localtime(test_begin)))
    print('test_end is: ', time.asctime(time.localtime(test_end)))
    f.close()
...
y_truth_valid, y_flat_64, y_flat_32, y_flat_16, y_pred_flat_64, y_pred_flat_32, y_pred_flat_16, total_loss, loss_list, learning_rate_current, train_step, accuracy_list, opt_vars_all, watcher_1, watcher_2, watcher_3 = input_data_LSTM.nt.net(x,y_,ef,isdrop,global_step, LEARNING_RATE_INIT, MOMENTUM_INIT, ITER_TIMES_PER_CHANGE_RATE,LEARNING_RATE_DECAY_RATIO, limit_grad = IS_TRAIN_LIMIT_GRAD, is_balance = IS_TRAIN_BALANCE) 

saver = tf.train.Saver(opt_vars_all, write_version=tf.train.SaverDef.V2)

if RUN_MODE == 1: # evaluate
    data_sets = input_data_LSTM.read_data_sets(0, 0, 0)
    saver.restore(sess, '/scratch/user/cherryxchen/HEVC/Inter_Pred/ETH-LSTM_Training_LDP_test/Models/LSTM/model_20210927_234933_200000_qp22.dat')
    evaluate()
    ...
    if IS_READ_DATA_SKIP == True:
        train_skip_samples = iter_times_last // ITER_TIMES_PER_CHANGE_TRAINSET * input_data_LSTM.TRAINSET_READSIZE
        valid_skip_samples = iter_times_last // ITER_TIMES_PER_CHANGE_VALIDSET * input_data_LSTM.VALIDSET_READSIZE
        data_sets = input_data_LSTM.read_data_sets(train_skip_samples, valid_skip_samples, 0)
    else:
        data_sets = input_data_LSTM.read_data_sets(0, 0, 0)
    ...
    for i in range(ITER_TIMES):
        ...
        if step % ITER_TIMES_PER_CHANGE_TRAINSET == 0 and i!=ITER_TIMES-1:
            input_data_LSTM.change_train_data_set(data_sets)
            
        if step % ITER_TIMES_PER_CHANGE_VALIDSET == 0 and i!=ITER_TIMES-1:
            input_data_LSTM.change_valid_data_set(data_sets)
