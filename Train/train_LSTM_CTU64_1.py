...
DATA_SWITCH = input_data_1.DATA_SWITCH
...
EVALUATE_QP_THR_LIST = input_data_1.EVALUATE_QP_THR_LIST
MODEL_NAME = input_data_1.MODEL_NAME
...
def get_class_matrices(y_truth, y_predict_64, y_predict_32, y_predict_16, thr_list):
	...
	for i in range(num_samples):
		class_64_truth=is_sep_64(y_truth[i], input_data_1.DEFAULT_THR_LIST[0])
		class_64_predict=is_sep_64(y_predict_64[i], thr_list[0])
		matrix_64[class_64_truth][class_64_predict] += 1
		if class_64_truth==1:
			for j in range(4):
				class_32_truth=is_sep_32(y_truth[i][index_32_list[j]], input_data_1.DEFAULT_THR_LIST[1])
				class_32_predict=is_sep_32(y_predict_32[i][j], thr_list[1])
				matrix_32[class_32_truth][class_32_predict] += 1
				if class_32_truth==1:
					for k in range(4):
						class_16_truth=is_sep_16(y_truth[i][index_32_list[j][k]], input_data_1.DEFAULT_THR_LIST[2])
...
def get_accuracy_on_large_data_interqp(f, data_set, qp_thr_list, thr_list, plot_tag_set=None):
	range_stat=input_data_1.RangingStatistics(qp_thr_list, 'scalar')
	qps = data_set.efs[:,1]
...
if RUN_MODE == 1: # evaluate
	...
	if IS_READ_DATA_SKIP == True:
		train_skip_samples = iter_times_last // ITER_TIMES_PER_CHANGE_TRAINSET * input_data_1.TRAINSET_READSIZE
		valid_skip_samples = iter_times_last // ITER_TIMES_PER_CHANGE_VALIDSET * input_data_1.VALIDSET_READSIZE
		data_sets = input_data_1.read_data_sets(train_skip_samples, valid_skip_samples, 0)
	else:
		data_sets = input_data_1.read_data_sets(0, 0, 0)
	...
	for i in range(ITER_TIMES):
		...
		if step % ITER_TIMES_PER_CHANGE_TRAINSET == 0 and i!=ITER_TIMES-1:
			input_data_1.change_train_data_set(data_sets)
			
		if step % ITER_TIMES_PER_CHANGE_VALIDSET == 0 and i!=ITER_TIMES-1:
			input_data_1.change_valid_data_set(data_sets)
	...
	savefig("Models/loss_accuracy_1_%s.png"%get_time_str()) 
	plt.close(fig)
	
	# save the data text in the file accuracy_arr.dat
	f = open('Models/loss_accuracy_list_1.dat', 'w+')
	...
	f.close()
	shutil.copyfile('Models/loss_accuracy_list_1.dat', 'Models/loss_accuracy_list_1_%s.dat'%get_time_str())
