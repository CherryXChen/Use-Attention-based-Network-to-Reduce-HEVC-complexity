...
def positional_encoding(inputs, maxlen,scope="positional_encoding"):
    '''
    inputs: 3d tensor. (N, T, 4, E)
    maxlen: scalar. Must be >= 4/16
    scope: `variable_scope`.
    '''

    E = inputs.get_shape().as_list()[-1] # static
    N, T, P= tf.shape(inputs)[0], tf.shape(inputs)[1],  tf.shape(inputs)[2]# dynamic

    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        # position indices
        position_ind = np.array([[j for i in range(int(np.sqrt(maxlen)))] for j in range(int(np.sqrt(maxlen)))])
        position_ind = tf.reshape(position_ind, [maxlen])
        position_ind_x = tf.tile(tf.expand_dims(tf.expand_dims(position_ind, 0), 0), [N, T, 1]) # (N, T, P)

        position_ind = np.array([[i for i in range(int(np.sqrt(maxlen)))] for j in range(int(np.sqrt(maxlen)))])
        position_ind = tf.reshape(position_ind, [maxlen])
        position_ind_y = tf.tile(tf.expand_dims(tf.expand_dims(position_ind, 0), 0), [N, T, 1])

        # First part of the PE function: sin and cos argument
        position_enc_x_i = np.array([
            [pos / np.power(300, (i-i%2)/(int(E/2))) for i in range((int(E/2)))]
            for pos in range(maxlen)])

        position_enc_y_i = np.array([
            [pos / np.power(300, (i-i%2)/(int(E/2))) for i in range((int(E/2)))]
            for pos in range(maxlen)])

        # Second part, apply the cosine to even columns and sin to odds.
        position_enc_x = position_enc_x_i
        position_enc_y = position_enc_y_i
        position_enc_x[:, 0::2] = np.sin(position_enc_x_i[:, 0::2])  # dim 2i
        position_enc_x[:, 1::2] = np.cos(position_enc_x_i[:, 1::2])  # dim 2i+1
        position_enc_y[:, 0::2] = np.sin(position_enc_y_i[:, 0::2])
        position_enc_y[:, 1::2] = np.cos(position_enc_y_i[:, 1::2])
        #position_enc = tf.concat([position_enc_x, position_enc_y], axis=-1)
        #position_enc = tf.cast(position_enc, tf.float32) # (maxlen, E)

        # lookup
        outputs_x = tf.nn.embedding_lookup(position_enc_x, position_ind_x)
        outputs_y = tf.nn.embedding_lookup(position_enc_y, position_ind_y)
        outputs = tf.concat([outputs_x, outputs_y], axis=-1)
...
def attention(x, num_x_size_x, num_input_hidden_size, isdrop,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        atten_inputs = ln(atten_inputs, num_x_size_x + 4)
